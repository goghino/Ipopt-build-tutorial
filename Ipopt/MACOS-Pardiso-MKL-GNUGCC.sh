...
# on MacOS (tested on Catalina) using Pardiso linear solver (v7.0)
# and Intel MKL library (v2020.1.216), using GNU GCC 10.2.0

################### Dependencies ##############################
# In this example we use the GNU GCC compiler suite, which is not available
# by default. It can be obtained from e.g. Homebrew (https://brew.sh/).
# In particular, we use version 10.2.0 (Homebrew GCC 10.2.0)
# Make sure you are able to run gcc-10 and g++-10 commands.

# Next, you need to install the Intel MKL library. You
...
# updates LD_LIBRARY_PATH and DYLD_LIBRARY_PATH variables.
# The path to the compilervars.sh script below might differ for you.
...
# obtain the MAC OS version (it should be named as e.g. libpardiso700-MACOS-X86-64.dylib).
...
export DYLD_LIBRARY_PATH=$PARDISO_PATH:$DYLD_LIBRARY_PATH
...
../configure F77=gfortran CC=gcc-10  CXX=g++-10  \
 --with-pic      \
 --prefix="$INSTALL_DIR" \
 --disable-java \
 --with-pardiso="-L$PARDISO_PATH -lpardiso700-MACOS-X86-64 -lgfortran -lgomp -lquadmath"
